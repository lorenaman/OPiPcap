...
def get_logger(name=None, level=logging.DEBUG, log_file='opi_pcap.log'):
    """
...
class OPi(object):
    ...
    def __init__(self, ipaddr='10.0.1.1', username='orangepi', password='orangepi', ssh_key='auth\id_rsa_putty.ppk', logger=None):
        self.ipaddr = ipaddr
    ...
    def is_pingable(self, ipaddr="10.0.1.1", cnt=1, wait=2000):
        ping_cmd = Template('ping -n $cnt -w $wait $ipaddr').substitute(cnt=cnt, wait=wait, ipaddr=ipaddr)
    ...
    def is_connected(self):
        ...
        # need another ssh connectivity check to confirm connectivity
        return True

    
class StatusCheckThread(threading.Thread):
    """
    Thread to check Device connectivity and wireshark running status in background
    """
    def __init__(self, opi_device, status_queue, logger, *args, **kwargs):
        threading.Thread.__init__(self, *args, **kwargs)
        self.opi_device = opi_device
    ...
    def run(self):
        ...
        prev_conn_status = ''
        prev_wireshark_status = ''
        
        while not self.stop_request.isSet():
            if self.opi_device.is_connected():
...
class ExecCmdThread(threading.Thread):
    """
    Thread to execute command on OPiPcap Device and return result
    """
    def __init__(self, opi_device, cmd_queue, res_queue, logger, *args, **kwargs):
        threading.Thread.__init__(self, *args, **kwargs)
        self.opi_device = opi_device
    ...
    def run(self):
        while not self.stop_request.isSet():
            if not self.cmd_queue.empty():
                ...
                success, stdout, stderr = self.opi_device.execute_cmd(mapping_cmd)
...
class OPiGUI(tk.Tk):
    """
    Class to create OPi-Pcap control panel
    ...
    def _create_body(self):
        SINGLE_BAND = OPi.CHANNELS_24G
        DUAL_BAND = OPi.CHANNELS_24G + OPi.CHANNELS_5G
        # device status frame -------------------------------
        ...
        self.btn_site_survey = tk.Button(self.packet_sniff, text='Start',
                                         command=lambda: self._send_cmd_to_thread('SITE_SURVEY', 30),
                                         bd=1, padx=3, pady=1)
        ...
        # device status  frame widgets
        self.lbl_device_shutdown = tk.Label(self.device_control, text='OPi-Pcap:', padx=3, pady=1)
        self.btn_device_shutdown = tk.Button(self.device_control, text='Shutdown Gracefully',
                                             ...
                                             bd=1, padx=3, pady=1, bg='SteelBlue1', width=13)
    ...
    def _show_site_survey_result(self, result):
        ...
        win_site_survey.title('OPi-Pcap')
        ...
        lbl_result = tk.Label(result_frame, text=result, justify=tk.LEFT)
        lbl_result.pack(side=tk.LEFT, padx=10, pady=5)
        
        bottom_frame = tk.Frame(win_site_survey)
        bottom_frame.grid(row=1, column=0, sticky='NEWS')

        btn_quit = tk.Button(bottom_frame, text='Exit', command=win_site_survey.destroy)
